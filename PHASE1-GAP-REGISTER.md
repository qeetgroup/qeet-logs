...
- *Outstanding Phase-1 P1 sub-items (revisit before Phase 1 exit):* metric rollups/downsampling 1m→5m→1h (2.4) and per-tenant label-cardinality rate-limit (currently only a per-minute request rate limit exists). Tracked, not silently dropped.
- *Deps:* G1.
...
- *Outstanding (tracked, P0):* **OTLP/protobuf** transport (`application/x-protobuf`) for logs/metrics/traces — OTLP/JSON covers all three signals today; protobuf is an additive transport needing the OTLP proto set vendored (focused follow-up before Phase-1 exit).
- *Deps:* G1–G3.
...
- *Outstanding (tracked):* conditionals + richer stdlib; route-before-index tiering (4.4, P1); extend remap to the OTLP/GELF/syslog paths (today JSON `/v1/ingest` + batch); per-tenant wall-clock budget (currently bounded by the no-loops design + statement cap).
- *Deps:* G5.
...
- *Outstanding (tracked):* topology-proximity as a correlation key (needs G9 wired into fingerprint — currently service+deploy); continuous calibration 13.3 = Phase 2.
- *Deps:* G4, G8, G9.
