...
- *~~Outstanding (tracked):~~* **Async buffering + auto-capture + env enrichment + Go SDK reconciliation** ✅ **DONE** — implemented in **`qeet-sdks/`**, the canonical SDK home (project convention: SDKs live in standalone `qeet-sdks/<product>-<lang>` repos, *not* this repo's `sdk/` tree, which is a legacy duplicate):
  - **`qeet-sdks/qeet-logs-go`** and **`qeet-sdks/qeet-logs-node`** gained a non-blocking **buffered ingester** (`Buffered(opts)` / `client.buffered()` → size- or interval-triggered batch to `/v1/ingest/batch`, with `Flush`/`Close`; drops on saturation, never stalls the caller), **env auto-enrichment** (`DetectDefaults`/`detectDefaults` — service/environment/git_sha/deploy_id/k8s from well-known env vars, Module 04.3), and **unhandled-exception/panic auto-capture** (`Recover` / `installErrorHandlers`). **`qeet-sdks/qeet-logs-react`** (read-only query/tail hooks; no ingest) verified.
  - The canonical Go SDK (`qeet-sdks/qeet-logs-go`) already uses the real two-host contract (flat `message`, NDJSON batch, stdlib WebSocket tail, `{columns,count,rows}` query) — the `{"records":…}`/`body` shape was only ever in the legacy in-repo `sdk/go`. The query API across all three was reconciled to send only the honored params (`q` + `format`); dead `from/to/limit/services/level` params removed. All three build/test green and are committed.
  - *Still deferred (Phase 2):* HTTP client/server auto-instrumentation (overlaps the OTel collector, G7).
- *Deps:* G5.
